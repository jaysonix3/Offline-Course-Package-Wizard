...
class CourseManagementApp:
    def __init__(self, root):
        ...
        self.question_bank_nd = tk.Label(self.buttons_frame,height=2,bg="#8A1538")
        self.question_bank_nd.grid(row=6, column=1,pady=10)

        self.create_quiz_nd = tk.Label(self.buttons_frame,height=2,bg="#8A1538")
        self.create_quiz_nd.grid(row=7, column=1,pady=10)

        self.create_ocp_nd = tk.Label(self.buttons_frame,height=2,bg="#8A1538")
        self.create_ocp_nd.grid(row=8, column=1,pady=10)
        ...
        self.question_bank_btn = tk.Button(self.buttons_frame,
                                    text="Question Bank",width=15,height=1,font=('Helvetica', 10),bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                                    borderwidth=0,anchor="w",state="disabled",
                                    command=lambda: self.indicate(self.question_bank_nd,self.question_bank_page))
        self.question_bank_btn.grid(row=6, column=2, columnspan=2,pady=10)
        ...
        self.create_quiz_btn.grid(row=7, column=2, columnspan=2,pady=10)
        ...
        self.create_ocp_btn.grid(row=8, column=2, columnspan=2,pady=10)
    ...
    def not_indicate(self):
        ...
        self.question_bank_nd.config(bg="#8A1538")
    ...
    def new_ocp(self):
        ...
        self.question_bank_btn.configure(state=tk.NORMAL)
    ...
    def course_info_page(self):
        ...
        course_no_entry=tk.Entry(course_info_page,font=('Helvetica',12),width=29,bg="#00573F",fg="#EEEEEE")
        ...
        course_title_entry=tk.Entry(course_info_page,font=('Helvetica',12),width=29,bg="#00573F",fg="#EEEEEE")
        ...
        save_btn=tk.Button(course_info_page,text="Save", font=('Helvetica',12),width=14,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda: self.save_course_info(clicked,course_no_entry, course_title_entry))
        save_btn.grid(row=4,column=2,sticky=tk.E,pady=10)

    def save_course_info(self, clicked, course_no_entry, course_title_entry):
    ...
    def course_intro_page(self):
        ...
        course_intro_label=tk.Label(course_intro_page, text="Course Introduction:",font=('Helvetica',12))
        ...
        course_intro_text=tk.Text(text_frame,yscrollcommand = scrollbar.set,width=45,height=10,font=('Helvetica',12),bg="#00573F",fg="#EEEEEE")
        ...
        save_btn=tk.Button(course_intro_page,text="Save", font=('Helvetica',12),width=14,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda: self.save_course_intro(course_intro_text))
        save_btn.grid(row=3,column=1,sticky=tk.E,pady=10)

    def save_course_intro(self, course_intro_text):
    ...
    def resources_page(self):
        ...
        course_guide_btn = tk.Button(resources_page,text="Upload Course Guide", font=('Helvetica',12),width=30,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda: self.save_course_guide())
        ...
        resources_btn = tk.Button(resources_page,text="Add Folders", font=('Helvetica',12),width=30,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda: self.save_folder())
    ...
    def save_folder(self):
        path = '{}'.format(filedialog.askdirectory(title='Select Folder'))
        if path:
            if [path,0] in self.resources_dir:
    ...
    def save_file(self):
        source = filedialog.askopenfilename(initialdir="/", title="Select file",
                                            filetypes=(("All Files", ""),))
        if source:
            if [source,1] in self.resources_dir:
    ...
    def remove_resources_page(self):
        ...
        for x in range(0,len(self.resources_dir)):
            lb.insert(x, self.resources_dir[x][0])
        ...
        delete_btn=tk.Button(remove_resources_page,text="Delete",font=('Helvetica',12),width=14,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                                    command=lambda: self.del_resources(lb))
        delete_btn.grid(row=2,column=1,sticky=tk.E)
        
    def del_resources(self,lb):
    ...
    def num_topics_page(self):
        ...
        vcmd = (self.main_frame.register(self.callback))

        num_topics_page=tk.Frame(self.main_frame)
        num_topics_page.place(relx=.5, rely=.5,anchor= tk.CENTER)

        num_topics_label=tk.Label(num_topics_page, text="No of Topics:",font=('Helvetica',12))
        num_topics_label.grid(row=1,column=1,sticky=tk.W,pady=10)

        num_topics_entry=tk.Entry(num_topics_page,justify=tk.CENTER,font=('Helvetica',12),width=29,bg="#00573F",fg="#EEEEEE",
        ...
        save_btn=tk.Button(num_topics_page,text="Save", font=('Helvetica',12),width=14,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda: self.save_num_topics(num_topics_entry))
    ...
    def save_num_topics(self,num_topics_entry):
        if self.num_topics == 0:
            self.update_num_topics(num_topics_entry)
    ...
    def update_num_topics(self,num_topics_entry):
        if int(num_topics_entry.get()) < 1:
            tk.messagebox.showerror("Error","Cannot be less than 1")
        else:
    ...
    def upload_topics_page(self):
        ...
        for x in range(self.num_topics):
            ...
            button = tk.Button(scrollable_frame, text=button_text,font=('Helvetica',12),width=30,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                            command=lambda value=x+1: self.upload_topics(value))
            button.grid(row=x+1,column=1,sticky=tk.E,pady=10)
            
    def upload_topics(self,value):
    ...
    def question_bank_page(self,quiz_no):
        self.destroy_main()
        self.preview_page = None

        with open(os.path.join(os.path.dirname(__file__),'template/course/qb.txt'), 'r') as file:
            self.content = file.readlines()
            qb_len = len(self.content)

        question_bank_page=tk.Frame(self.main_frame)
        question_bank_page.place(relx=.5, rely=.5,anchor= tk.CENTER)
        
        scrollable_frame = self.create_scroll_frame(question_bank_page)

        # List to store the selected questions
        self.selected_questions = []

        # Dictionary to track the selected state for each question
        self.question_selection = {x: tk.BooleanVar() for x in range(qb_len)}

        for x in range(qb_len):
            # Create a checkbox for each question
            checkbox = tk.Checkbutton(scrollable_frame, variable=self.question_selection[x])
            preview_btn=tk.Button(scrollable_frame,text="P", font=('Helvetica',12),width=1,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda value=x+1: self.preview_question(value-1,"back"))
            
            edit_btn=tk.Button(scrollable_frame,text="E", font=('Helvetica',12),width=1,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda value=x+1: self.preview_question(value-1,"edit"))
            
            if len(self.deobfuscate(self.content[x])) > 35:
                temp_text = self.deobfuscate(self.content[x])['title'][0:32] + "..."
            else:
                temp_text = self.deobfuscate(self.content[x])['title']

            question_label=tk.Label(scrollable_frame,text=temp_text,highlightbackground='black',highlightthickness=2,font=('Helvetica',12),anchor=tk.W)
            
            checkbox.grid(row=x + 1, column=0, padx=5, pady=5)
            preview_btn.grid(row=x + 1, column=1, padx=5, pady=5)
            edit_btn.grid(row=x + 1, column=2, padx=5, pady=5)
            question_label.grid(row=x + 1, column=3, sticky="EW", padx=5, pady=5, columnspan=2)
            scrollable_frame.grid_columnconfigure(3, weight=1)

        add_btn = tk.Button(question_bank_page,text="Add Question", font=('Helvetica',12),width=14,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda value=len(self.content): self.preview_question(value,"add"))
        add_btn.grid(row=2,column=1,sticky=tk.W)

        add_btn2 = tk.Button(question_bank_page,text="Save ", font=('Helvetica',12),width=14,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                        command=lambda: self.add_selected_questions(quiz_no))
        add_btn2.grid(row=2,column=2,sticky=tk.W)        

    def add_selected_questions(self,quiz_no):
        # Clear the list of selected questions
        self.selected_questions.clear()

        # Iterate through the questions and add the selected ones to the list
        for x, var in self.question_selection.items():
            if var.get():
                self.selected_questions.append(self.content[x].strip())

        list_question = []
        for x in self.selected_questions:
            temp = self.deobfuscate(x)
            del temp['title']
    ...
    def deobfuscate(self,q):
        encoded_questions_json = base64.b64decode(q).decode()
        question = json.loads(encoded_questions_json)
        return question

    def preview_question(self,value,type):
        if self.preview_page is not None:
            self.preview_page.destroy()
        self.preview_page=tk.Toplevel(self.main_frame)
        self.preview_page.title("Question Preview")

        topic_title = tk.Label(self.preview_page, text=f"Title",font=('Helvetica',12))
        topic_title.grid(row=0,column=1,sticky=tk.W,pady=10)
        
        topic_entry=tk.Entry(self.preview_page,width=10,font=('Helvetica',12),bg="#00573F",fg="#EEEEEE")
        topic_entry.grid(row=1,column=1,sticky=tk.EW)
        #QUESTIONS ------------------------------------------------------------------------------------------------
        question_label = tk.Label(self.preview_page, text=f"Question",font=('Helvetica',12))
        question_label.grid(row=2,column=1,sticky=tk.W,pady=10)

        text_frame1=tk.Frame(self.preview_page)

        scrollbar1=tk.Scrollbar(text_frame1)

        question_text=tk.Text(text_frame1,yscrollcommand = scrollbar1.set,width=39,height=3,font=('Helvetica',12),bg="#00573F",fg="#EEEEEE")
        question_text.grid(row=0,column=0)

        scrollbar1.grid(row=0,column=1,sticky="NS")
        scrollbar1.config( command = question_text.yview )

        text_frame1.grid(row=3, column=1, pady=10)
        #CHOICES ------------------------------------------------------------------------------------------------
        choices_label = tk.Label(self.preview_page, text="Choices",font=('Helvetica',12))
        choices_label.grid(row=4,column=1,sticky=tk.W,pady=10)

        text_frame2=tk.Frame(self.preview_page)

        scrollbar2=tk.Scrollbar(text_frame2)

        choices_text=tk.Text(text_frame2,yscrollcommand = scrollbar2.set,width=39,height=3,font=('Helvetica',12),bg="#00573F",fg="#EEEEEE")
        choices_text.grid(row=0,column=0)

        scrollbar2.grid(row=0,column=1,sticky="NS")
        scrollbar2.config( command = choices_text.yview )

        text_frame2.grid(row=5, column=1, pady=10)
        #ANSWERS ------------------------------------------------------------------------------------------------
        answers_label = tk.Label(self.preview_page, text="Correct Answer(s)",font=('Helvetica',12))
        answers_label.grid(row=6,column=1,sticky=tk.W,pady=10)

        text_frame3=tk.Frame(self.preview_page)

        scrollbar3=tk.Scrollbar(text_frame3)

        answers_text=tk.Text(text_frame3,yscrollcommand = scrollbar3.set,width=39,height=3,font=('Helvetica',12),bg="#00573F",fg="#EEEEEE")
        answers_text.grid(row=0,column=0)

        scrollbar3.grid(row=0,column=1,sticky="NS")
        scrollbar3.config( command = choices_text.yview )

        text_frame3.grid(row=7, column=1, pady=10)
        # print(self.content)

        if type!="add":
            if self.content[value] != None:
                encrypted_questions = self.content[value]
                decoded_questions_json = base64.b64decode(encrypted_questions).decode()
                questions = json.loads(decoded_questions_json)
                # print(questions)
                if questions['title']:
                    topic_entry.insert(0, questions['title'])
                if questions['label']:
                    question_text.insert(tk.END, questions['label'])
                if questions['options']:
                    for y in range(len(questions['options'])):
                        if y+1 != len(questions['options']):
                            choices_text.insert(tk.END, questions['options'][y] + "\n")
                        else:
                            choices_text.insert(tk.END, questions['options'][y])
                if questions['answer']:
                    for y in range(len(questions['answer'])):
                        if y+1 != len(questions['answer']):
                            answers_text.insert(tk.END, questions['answer'][y] + "\n")
                        else:
                            answers_text.insert(tk.END, questions['answer'][y])
        if type=="back":
            topic_entry.configure(state=tk.DISABLED)
        ...
        if type=="edit" or type=="add":
            back_btn=tk.Button(self.preview_page,text="Save", font=('Helvetica',12),width=14,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                            command=lambda: self.edit_question(value,topic_entry,question_text,choices_text,answers_text,type))
            back_btn.grid(row=8, column=1, sticky=tk.E)
        elif type=="back":
            back_btn=tk.Button(self.preview_page,text="Back", font=('Helvetica',12),width=14,bg="#8A1538",fg="#EEEEEE",activebackground='#975361',
                            command=lambda: self.indicate(self.question_bank_nd, self.question_bank_page))
            back_btn.grid(row=8, column=1, sticky=tk.E)

    def edit_question(self,value,title,label,options,answer,type):
        temp_dict={}
        title=title.get()
        question=label.get("1.0", 'end-1c').strip()
        choices_temp=options.get("1.0", 'end-1c').strip()
        choices = [choice.strip() for choice in choices_temp.split('\n') if choice.strip()]
        # random.shuffle(choices)
        # # print(choices)
        answers_temp=answer.get("1.0", 'end-1c').strip()
        answers = [answer.strip() for answer in answers_temp.split('\n') if answer.strip()]
        if question == "" or not choices or not answers:
            tk.messagebox.showerror('Error',f'Please fill all fields.\nMissing Field on Question')
        elif len(choices) <= 1:
            tk.messagebox.showerror('Error', f"Must have more than one choice.")
        elif set(answers).issubset(set(choices)) != True:
            tk.messagebox.showerror('Error', f'Correct Answer(s) not in Choices.\nPlease check spelling and Caps')
        else:
            temp_dict={
                'title':title,
                'label':question,
            ...
            if temp_dict != None:
                questions_json = json.dumps(temp_dict)
                encoded_questions = base64.b64encode(questions_json.encode()).decode()+'\n'
                if type=="edit":
                    self.content[value]=encoded_questions
                    with open(os.path.join(os.path.dirname(__file__),'template/course/qb.txt'), 'w') as file:
                        file.writelines(self.content)
                    self.indicate(self.question_bank_nd, self.create_quiz_page)
                elif type=="add":
                    self.content.append(encoded_questions)
                    with open(os.path.join(os.path.dirname(__file__),'template/course/qb.txt'), 'w') as file:
                        file.writelines(self.content)
                    self.indicate(self.question_bank_nd, self.create_quiz_page)
                else:
    ...
    def create_quiz_page(self):
        ...
        def button_click(quiz_no):
            self.question_bank_page(quiz_no)
    ...
    def num_questions_page(self,quiz_no):
        ...
        num_questions_label=tk.Label(num_questions_page, text="No of Questions: ",font=('Helvetica',12))
        num_questions_label.grid(row=1,column=1,sticky=tk.W,pady=10)

        num_questions_entry=tk.Entry(num_questions_page,justify=tk.CENTER,font=('Helvetica',12),width=29,bg="#00573F",fg="#EEEEEE")
    ...
    #         #QUESTIONS ------------------------------------------------------------------------------------------------
    #         question_label = tk.Label(question_frame, text=f"Question {x+1}",font=('Helvetica',12))
    ...
    #         #CHOICES ------------------------------------------------------------------------------------------------
    #         choices_label = tk.Label(question_frame, text="Choices",font=('Helvetica',12))
    ...
    #         #ANSWERS ------------------------------------------------------------------------------------------------
    #         answers_label = tk.Label(question_frame, text="Correct Answer(s)",font=('Helvetica',12))
    ...
    def create_ocp_page(self):
        ...
        row_cnt=1
        # Faculty Label
        faculty_label = tk.Label(scrollable_frame, text="Faculty: ", font=('Helvetica', 12))
        faculty_label.grid(row=row_cnt, column=1, pady=10, sticky=tk.W)
        faculty_data = tk.Label(scrollable_frame, text=self.faculty, font=('Helvetica', 12),fg="#8A1538")
        faculty_data.grid(row=row_cnt, column=2, pady=10,sticky=tk.W)
        row_cnt+=1
        # Course No Label
        course_no_label = tk.Label(scrollable_frame, text="Course No: ", font=('Helvetica', 12))
        course_no_label.grid(row=row_cnt, column=1, pady=10, sticky=tk.W)
        course_no_data = tk.Label(scrollable_frame, text=self.course_no, font=('Helvetica', 12),fg="#8A1538")
        course_no_data.grid(row=row_cnt, column=2, pady=10,sticky=tk.W)
        row_cnt+=1
        # Course Title Label
        course_title_label = tk.Label(scrollable_frame, text="Course Title: ", font=('Helvetica', 12))
        course_title_label.grid(row=row_cnt, column=1, pady=10, sticky=tk.W)
        course_title_data = tk.Label(scrollable_frame, text=self.course_title, font=('Helvetica', 12),fg="#8A1538")
        course_title_data.grid(row=row_cnt, column=2, pady=10,sticky=tk.W)
        row_cnt+=1
        # Course Intro Label
        course_intro_label = tk.Label(scrollable_frame, text="Course Intro: ", font=('Helvetica', 12))
        course_intro_label.grid(row=row_cnt, column=1, sticky=tk.W)
        if self.course_intro:
            for i in range(len(self.course_intro)):
                if i+1 == len(self.course_intro):
                    course_intro_data = tk.Label(scrollable_frame,text=self.course_intro[i], font=('Helvetica', 12),fg="#8A1538")
        ...
        # Course Dir Label
        course_guide_dir_label = tk.Label(scrollable_frame, text="Course Guide: ", font=('Helvetica', 12))
        course_guide_dir_label.grid(row=row_cnt, column=1, pady=10, sticky=tk.W)
        course_guide_dir_data = tk.Label(scrollable_frame, text=self.course_guide_dir, font=('Helvetica', 12),fg="#8A1538")
        course_guide_dir_data.grid(row=row_cnt, column=2, pady=10,sticky=tk.W)
        row_cnt+=1

        #Resources
        resources_dir_label = tk.Label(scrollable_frame, text="Resources: ", font=('Helvetica', 12))
        ...
        if self.resources_dir:
            for i in range(len(self.resources_dir)):
                if i+1 == len(self.resources_dir):
                    resources_dir_data = tk.Label(scrollable_frame,text=self.resources_dir[i][0], font=('Helvetica', 12),fg="#8A1538")
        ...
        if bool(self.topics_dir) != False:
            ...
            for i in range(len(temp)):
                if i+1 == len(temp):
                    topics_dir_label = tk.Label(scrollable_frame,text=f"Topic {temp[i]} Dir", font=('Helvetica', 12))
                    topics_dir_label.grid(row=row_cnt, column=1, pady=(0,10),sticky=tk.W)

                    topics_dir_data = tk.Label(scrollable_frame,text=self.topics_dir[int(temp[i])], font=('Helvetica', 12),fg="#8A1538")
